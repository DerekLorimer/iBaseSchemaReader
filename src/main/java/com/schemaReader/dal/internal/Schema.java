...
public class Schema implements ISchema {
	...
	public Schema(EntityManager entityManager) {
		...
		itemTypes.stream().sorted().forEach(itemType -> mapTables.put(itemType.getId(), itemType));

		schemaReader.readTable(Field.class, entityManager, criteriaBuilder).forEach(field -> {
